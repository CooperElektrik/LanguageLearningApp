...
class CourseTreeWidget(QTreeWidget):
    ...
    def __init__(self, course_data: Any, parent=None):
        ...
        self.setSelectionMode(QTreeWidget.SingleSelection) # Ensure only one item dragged at a time
    ...
    def dropMimeData(self, parent_item: QTreeWidgetItem, index: int, data: QMimeData, action: Qt.DropAction) -> bool:
        """
        ...
        if parent_item:
            ...
            if new_parent_obj is None: # Dropped on manifest item or other non-data item
                logger.warning(f"Drop on invalid parent item: {parent_item.text(0)}")
                QMessageBox.warning(self, "Invalid Drop Location", "Cannot drop items here.")
                return False
        ...
        if not is_valid_drop:
            QMessageBox.warning(self, "Invalid Move", f"A {type(dragged_obj).__name__} cannot be dropped into a {type(new_parent_obj).__name__ if new_parent_obj else 'top-level'}.")
            return False
        ...
        if target_list is None:
            logger.error(f"Could not determine target list for new parent {new_parent_obj}.")
            QMessageBox.critical(self, "Internal Error", "Could not determine target list for drop.")
            return False
        ...
        if original_list is target_list:
            if original_index < new_insert_index:
                new_insert_index -= 1 # Adjust index if removing from before insert point
            ...
            if original_list and dragged_obj in original_list: # Ensure it's still there
                original_list.remove(dragged_obj) # Remove by value, not index
            else:
                 logger.error(f"Dragged object {dragged_obj_id} not found in its original list during removal.")
                 return False # Should not happen if _find_obj_and_context_by_id is robust
    ...
    def _find_obj_and_context_by_id(self, obj_id: int, current_obj: Any,
                                    parent_obj: Any = None, current_list: Optional[List[Any]] = None) -> tuple[Any, Any, Optional[List[Any]], int]:
        """
        ...
        if id(current_obj) == obj_id:
            return current_obj, parent_obj, current_list, current_list.index(current_obj) if current_list else -1
        ...
        if isinstance(current_obj, (Course, Unit, Lesson)):
            if hasattr(current_obj, 'units') and isinstance(current_obj.units, list):
                for i, unit in enumerate(current_obj.units):
                    found_obj, p_obj, l_list, idx = self._find_obj_and_context_by_id(obj_id, unit, current_obj, current_obj.units)
                    if found_obj: return found_obj, p_obj, l_list, idx
            
            if hasattr(current_obj, 'lessons') and isinstance(current_obj.lessons, list):
                for i, lesson in enumerate(current_obj.lessons):
                    found_obj, p_obj, l_list, idx = self._find_obj_and_context_by_id(obj_id, lesson, current_obj, current_obj.lessons)
                    if found_obj: return found_obj, p_obj, l_list, idx
            
            if hasattr(current_obj, 'exercises') and isinstance(current_obj.exercises, list):
                for i, exercise in enumerate(current_obj.exercises):
                    found_obj, p_obj, l_list, idx = self._find_obj_and_context_by_id(obj_id, exercise, current_obj, current_obj.exercises)
                    if found_obj: return found_obj, p_obj, l_list, idx
