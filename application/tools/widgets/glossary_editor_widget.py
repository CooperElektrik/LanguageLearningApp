...
class GlossaryEditorWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_manifest_path: str = None # Needed to get course root dir for asset dialog
    ...
    def load_glossary_data(self, manifest_data: Dict[str, Any], manifest_path: str):
        ...
        if glossary_filename:
            ...
            self.current_glossary_filepath = os.path.join(manifest_dir_abs, glossary_filename)
            ...
            QMessageBox.information(self, "Glossary Missing", "Manifest does not specify a 'glossary_file'. Create a new one or add to manifest.")
            # Optionally, prompt to create a new glossary file if not specified
            self.data_changed.emit() # Even if empty, reflect state

        self._populate_list_widget()

    def save_glossary_data(self, manifest_data: Dict[str, Any], manifest_path: str) -> bool:
        """
        ...
        if not self.current_glossary_filepath:
            ...
            self.current_glossary_filepath = os.path.join(manifest_dir_abs, default_glossary_filename)
            # Update the manifest data with the new glossary file name
            manifest_data["glossary_file"] = default_glossary_filename
            logger.info(f"New glossary file '{default_glossary_filename}' will be saved and linked in manifest.")
            # Trigger manifest save after this, via EditorWindow's _do_save

        if not self.current_glossary_filepath: # Should not happen if above logic is good
            logger.error("No glossary filepath determined for saving.")
            return False

        return yaml_serializer.save_glossary_to_yaml(self.glossary_entries, self.current_glossary_filepath)
    ...
    def _add_entry(self):
        dialog = GlossaryEntryEditorDialog(course_root_dir=os.path.dirname(self.current_manifest_path), parent=self)
    ...
    def _edit_selected_entry(self):
        ...
        if not selected_items:
            QMessageBox.information(self, "Edit Entry", "Please select an entry to edit.")
            return

        current_item = selected_items[0]
        entry_to_edit: GlossaryEntry = current_item.data(Qt.UserRole)

        dialog = GlossaryEntryEditorDialog(entry=entry_to_edit, course_root_dir=os.path.dirname(self.current_manifest_path), parent=self)
        if dialog.exec() == QDialog.Accepted:
    ...
    def _delete_selected_entry(self):
        selected_items = self.glossary_list_widget.selectedItems()
        if not selected_items:
            QMessageBox.information(self, "Delete Entry", "Please select an entry to delete.")
            return
