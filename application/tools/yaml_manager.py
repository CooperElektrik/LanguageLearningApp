...
# Assume 'application' directory is on Python path or handled by main_editor.py
# (This import path setup is critical)
try:
    from core.models import Course, Unit, Lesson, Exercise, ExerciseOption
except ImportError:
    # Fallback/debug in case run in isolation; main_editor will handle sys.path
    logging.warning("Could not import models directly. Ensure application/core is on sys.path for standalone testing.")
    # Define dummy models if necessary for local testing, or raise an error
    class Course: pass
    class Unit: pass
    class Lesson: pass
    class Exercise: pass
...
def save_manifest(manifest_data: Dict[str, Any], manifest_path: str):
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred while saving manifest {manifest_path}: {e}")
    return False

# --- Course Content Operations ---

def _parse_exercise_data_to_model(ex_data: Dict[str, Any], lesson_id: str, index: int) -> Exercise:
    ...
    if 'options' in ex_data and isinstance(ex_data['options'], list):
        for opt_data in ex_data['options']:
            if isinstance(opt_data, dict): # For MC
                options_list.append(ExerciseOption(text=opt_data['text'], correct=opt_data.get('correct', False)))
            elif isinstance(opt_data, str): # For FIB (correct flag is separate)
                options_list.append(ExerciseOption(text=opt_data, correct=(opt_data == ex_data.get('correct_option'))))
                
    return Exercise(
        exercise_id=f"{lesson_id}_ex{index}_{uuid.uuid4().hex[:4]}", # Generate unique ID for editor
        ...
        raw_data=ex_data # Keep original raw data for fidelity if needed
    )

def load_course_content_from_yaml(content_filepath: str, manifest_data: Dict[str, Any]) -> Optional[Course]:
    ...
    for unit_data in raw_course_data.get('units', []):
        unit_obj = Unit(unit_id=unit_data.get('unit_id', str(uuid.uuid4())), title=unit_data.get('title', ''))
        for lesson_data in unit_data.get('lessons', []):
            lesson_obj = Lesson(lesson_id=lesson_data.get('lesson_id', str(uuid.uuid4())), title=lesson_data.get('title', ''), unit_id=unit_obj.unit_id)
            for i, ex_data in enumerate(lesson_data.get('exercises', [])):
                exercise_obj = _parse_exercise_data_to_model(ex_data, lesson_obj.lesson_id, i)
...
def save_course_content_to_yaml(course: Course, content_filepath: str):
    ...
    try:
        # The Course.to_dict() method is designed to produce the 'units' structure
        # suitable for the content YAML file.
        ...
        logger.error(f"An unexpected error occurred while saving course content {content_filepath}: {e}")
    return False
