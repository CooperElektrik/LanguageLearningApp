...
# --- Validation Helper Functions (now accept errors_list) ---
...
def _validate_exercise_translation_internal(exercise: Exercise, path_prefix: str, errors_list: List[str]):
    if not exercise.prompt or not isinstance(exercise.prompt, str) or not exercise.prompt.strip():
        _collect_error(errors_list, "Missing or empty 'prompt' (must be a string).", path_prefix)
    if not exercise.answer or not isinstance(exercise.answer, str) or not exercise.answer.strip():
        _collect_error(errors_list, "Missing or empty 'answer' (must be a string).", path_prefix)

def _validate_exercise_mcq_internal(exercise: Exercise, path_prefix: str, errors_list: List[str]):
    if not exercise.source_word or not isinstance(exercise.source_word, str) or not exercise.source_word.strip():
        _collect_error(errors_list, "Missing or empty 'source_word' (must be a string).", path_prefix)
    
    if not exercise.options or not isinstance(exercise.options, list) or len(exercise.options) == 0:
        _collect_error(errors_list, "Missing, empty, or invalid 'options' list.", path_prefix)
        return
    ...
    for i, option in enumerate(exercise.options):
        ...
        if not hasattr(option, 'text') or not isinstance(option.text, str) or not option.text.strip():
             _collect_error(errors_list, "Missing or empty 'text' for option.", option_path)
        if not hasattr(option, 'correct') or not isinstance(option.correct, bool):
             _collect_error(errors_list, "Missing or invalid 'correct' flag (must be boolean) for option.", option_path)
    ...
    if correct_count == 0:
        ...
        _collect_error(errors_list, f"Multiple options ({correct_count}) marked as correct. Only one should be.", path_prefix)

def _validate_exercise_fib_internal(exercise: Exercise, path_prefix: str, errors_list: List[str]):
    if not exercise.sentence_template or not isinstance(exercise.sentence_template, str) or not exercise.sentence_template.strip():
        _collect_error(errors_list, "Missing or empty 'sentence_template' (must be a string).", path_prefix)
    elif "__BLANK__" not in exercise.sentence_template:
        _collect_error(errors_list, "Warning: 'sentence_template' does not contain '__BLANK__' placeholder (recommended).", path_prefix)

    if not exercise.correct_option or not isinstance(exercise.correct_option, str) or not exercise.correct_option.strip():
        _collect_error(errors_list, "Missing or empty 'correct_option' (must be a string).", path_prefix)
    
    if not exercise.options or not isinstance(exercise.options, list) or len(exercise.options) == 0:
        _collect_error(errors_list, "Missing, empty, or invalid 'options' list for blank choices.", path_prefix)
        return

    option_texts = [opt.text for opt in exercise.options if hasattr(opt, 'text') and opt.text.strip()]
    if exercise.correct_option and exercise.correct_option.strip() and exercise.correct_option.strip() not in option_texts:
        _collect_error(errors_list, f"'correct_option' ('{exercise.correct_option}') not found in the provided 'options' list texts.", path_prefix)
...
def _validate_exercise_internal(exercise: Exercise, index: int, lesson_path_prefix: str, errors_list: List[str]):
    ...
    if not hasattr(exercise, 'type') or not isinstance(exercise.type, str) or not exercise.type.strip():
        _collect_error(errors_list, "Missing or empty 'type' field.", path_prefix)
        return # Cannot validate further without type

    valid_types = ["translate_to_target", "translate_to_source", "multiple_choice_translation", "fill_in_the_blank"]
    if exercise.type not in valid_types:
        _collect_error(errors_list, f"Invalid exercise type '{exercise.type}'. Must be one of {valid_types}.", path_prefix)
        return
...
def _validate_lesson_internal(lesson: Lesson, unit_path_prefix: str, errors_list: List[str]):
    ...
    if not hasattr(lesson, 'title') or not isinstance(lesson.title, str) or not lesson.title.strip():
    ...
    for i, exercise in enumerate(lesson.exercises):
        _validate_exercise_internal(exercise, i, path_prefix, errors_list)

def _validate_unit_internal(unit: Unit, course_path_prefix: str, errors_list: List[str]):
    ...
    if not hasattr(unit, 'title') or not isinstance(unit.title, str) or not unit.title.strip():
        _collect_error(errors_list, "Missing or empty 'title'.", path_prefix)
    ...
    for lesson in unit.lessons:
        if not hasattr(lesson, 'lesson_id') or not isinstance(lesson.lesson_id, str): # Pre-check for duplicate logic
            _collect_error(errors_list, "Lesson has invalid or missing lesson_id, cannot check for duplicates.", path_prefix)
        ...
        _validate_lesson_internal(lesson, path_prefix, errors_list)


# --- New Callable Functions for Editor (replace old CLI main logic) ---

def perform_manifest_validation(manifest_data: Dict[str, Any], manifest_path: str) -> List[str]:
    """Validates the loaded manifest data, returning a list of errors."""
    errors = []
    required_fields = ["course_id", "course_title", "target_language", "source_language", "content_file"]
    for field in required_fields:
        if field not in manifest_data or not manifest_data[field] or (isinstance(manifest_data[field], str) and not manifest_data[field].strip()):
            _collect_error(errors, f"Manifest missing or empty required field: '{field}'")
    
    if "content_file" in manifest_data and manifest_data["content_file"]:
        content_filepath = os.path.join(os.path.dirname(manifest_path), manifest_data["content_file"])
...
def perform_course_content_validation(course: Course) -> List[str]:
    ...
    if not course or not hasattr(course, 'units') or not isinstance(course.units, list):
        _collect_error(errors, "Course object is invalid or has no 'units' list.", path_prefix)
        return errors
    ...
    for unit in course.units:
        if not hasattr(unit, 'unit_id') or not isinstance(unit.unit_id, str): # Pre-check for duplicate logic
            _collect_error(errors, "Unit has invalid or missing unit_id, cannot check for duplicates.", path_prefix)
        ...
        _validate_unit_internal(unit, path_prefix, errors)
...
def main():
    ...
    current_errors: List[str] = [] # Use local list for CLI
    ...
    if not manifest_data:
        _collect_error(current_errors, "Failed to load or parse manifest. See previous logs for details.", "")
    else:
        manifest_validation_errors = perform_manifest_validation(manifest_data, args.manifest_file)
        current_errors.extend(manifest_validation_errors)

    # 2. Load and Validate Course Content (if manifest was minimally valid to get content_file)
    course_obj: Optional[Course] = None
    if manifest_data and "content_file" in manifest_data and manifest_data["content_file"]:
        ...
        if os.path.exists(content_filepath):
            ...
            pass 
    elif manifest_data: # Manifest loaded but no content_file or it's empty
        _collect_error(current_errors, "Manifest is missing 'content_file' or it is empty; cannot validate course content.", "")
    ...
    if not current_errors:
        ...
        for i, err in enumerate(current_errors):
           logger.error(f"  {i+1}. {err}") # Log to console as error
        logger.info("---------------------------------------------")
        sys.exit(1)
