...
def create_package_for_gui(
    ...
    if not manifest_data: return False, None, messages
    course_id = manifest_data.get("course_id")
    version = manifest_data.get("version", "unknown")
    content_filename = manifest_data.get("content_file")
    if not all([course_id, content_filename]): # Basic check
        messages.append("Error: Manifest missing critical fields (course_id or content_file).")
    ...
    if not os.path.exists(abs_content_filepath):
        messages.append(f"Error: Course content file '{content_filename}' not found at '{abs_content_filepath}'.")
        return False, None, messages

    # --- Load course content to find asset files ---
    ...
    try:
        ...
        messages.append(f"Error: Could not read course content file '{abs_content_filepath}' to find assets: {e}")
        return False, None, messages
    # --- End asset loading ---

    # ... (Determine output_package_path as before) ...
    if package_name_override and package_name_override.strip():
        final_package_name_stem = package_name_override.strip()
    else:
        final_package_name_stem = f"{str(course_id).replace(' ', '_')}_{str(version).replace(' ', '_')}"
    ...
    if not os.path.exists(output_dir_to_use):
        try: os.makedirs(output_dir_to_use, exist_ok=True); messages.append(f"Info: Created output directory: {output_dir_to_use}")
        except OSError as e: messages.append(f"Error: Could not create output directory '{output_dir_to_use}': {e}"); return False, None, messages
    ...
    try:
        with zipfile.ZipFile(output_package_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            # --- Add Assets (Audio/Images) ---
            assets_to_add = set() # Use a set to avoid duplicate additions
            if course_content_data and 'units' in course_content_data:
                for unit in course_content_data.get('units', []):
                    for lesson in unit.get('lessons', []):
                        for exercise in lesson.get('exercises', []):
                            if isinstance(exercise, dict): # Ensure exercise is a dict
                                audio_path = exercise.get('audio_file')
                                image_path = exercise.get('image_file') # For future image support
            ...
            for asset_relative_path in assets_to_add:
                # asset_relative_path is like "sounds/hello.mp3" or "images/cat.png"
                # It's relative to the course content file (which we assume is in manifest_dir for simplicity here)
                abs_asset_path = os.path.join(manifest_dir, asset_relative_path)
                if os.path.exists(abs_asset_path):
                    # Add to zip, maintaining the relative path structure (e.g., sounds/hello.mp3 inside zip)
                    zf.write(abs_asset_path, arcname=asset_relative_path)
                    messages.append(f"Info: Added asset '{asset_relative_path}' to package.")
                else:
                    messages.append(f"Warning: Asset file '{asset_relative_path}' referenced in course content not found at '{abs_asset_path}'. Not added to package.")
            # --- End Add Assets ---
