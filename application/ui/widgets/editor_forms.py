...
class BaseEditorForm(QWidget):
    ...
    def _populate_fields(self):
        ...
        logger.debug(
            f"Calling _populate_fields in {self.__class__.__name__} for {self.data_object}"
        )
...
class UnitEditorForm(BaseEditorForm):
    ...
    def _populate_fields(self):
        if isinstance(self.data_object, Unit):
            logger.debug(
                f"Populating UnitEditorForm with Unit ID: {self.data_object.unit_id}, Title: {self.data_object.title}"
            )
            self.id_input.setText(self.data_object.unit_id or "")
            self.title_input.setText(self.data_object.title or "")
        else:
            logger.warning(
                f"Attempted to populate UnitEditorForm with non-Unit object: {type(self.data_object)}"
            )
    ...
    def _on_id_changed(self, text):
        if self.data_object:
            self.data_object.unit_id = text
            self.data_changed.emit()
            logger.debug(f"Unit ID changed to: {text}")

    def _on_title_changed(self, text):
        if self.data_object:
            self.data_object.title = text
            self.data_changed.emit()
            logger.debug(f"Unit Title changed to: {text}")
...
class LessonEditorForm(BaseEditorForm):
    ...
    def _populate_fields(self):
        if isinstance(self.data_object, Lesson):
            logger.debug(
                f"Populating LessonEditorForm with Lesson ID: {self.data_object.lesson_id}, Title: {self.data_object.title}"
            )
            self.id_input.setText(self.data_object.lesson_id or "")
            self.title_input.setText(self.data_object.title or "")
        else:
            logger.warning(
                f"Attempted to populate LessonEditorForm with non-Lesson object: {type(self.data_object)}"
            )
    ...
    def _on_id_changed(self, text):
        if self.data_object:
            self.data_object.lesson_id = text
            self.data_changed.emit()
            logger.debug(f"Lesson ID changed to: {text}")

    def _on_title_changed(self, text):
        if self.data_object:
            self.data_object.title = text
            self.data_changed.emit()
            logger.debug(f"Lesson Title changed to: {text}")
...
class ExerciseEditorForm(BaseEditorForm):
    ...
    def _populate_fields(self):
        if not isinstance(self.data_object, Exercise):
            logger.warning(
                f"Attempted to populate ExerciseEditorForm with non-Exercise object: {type(self.data_object)}"
            )
            return
        ...
        logger.debug(
            f"Populating ExerciseEditorForm for exercise type: {ex.type}, title: {ex.title}"
        )
        ...
        if ex.words:
            logger.debug(f"Populated {len(ex.words)} jumble words.")
    ...
    def _on_type_changed(self, new_type: str):
        if self.data_object:
            ...
            logger.debug(
                f"Exercise type changed to: {new_type}. Updating visible fields."
            )
    ...
    def _on_field_changed(self):
        ...
        ex.words = [
            line for line in self.words_input.toPlainText().split("\n") if line.strip()
        ]
        ...
        for i in range(self.options_list.count()):
            ...
            ex.options.append(
                ExerciseOption(
                    text=item.text(), correct=item.checkState() == Qt.Checked
                )
            )
        logger.debug(
            f"Exercise field changed. Title: {ex.title}, Prompt: {ex.prompt is not None}, Answer: {ex.answer}, Audio: {ex.audio_file}, Image: {ex.image_file}, NumWords: {len(ex.words)}, NumOptions: {len(ex.options)}"
        )
        self.data_changed.emit()
